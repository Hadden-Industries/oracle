...
CREATE OR REPLACE
PROCEDURE REFRESH_COUNTRYSUBDIVNAME
AS
    
    --Email variables
    vSubject VARCHAR2(78 CHAR) := 'COUNTRYSUBDIVNAME ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD"T"HH24:MI:SS');
...
BEGIN
    ...
    BEGIN
        ...
        || '<td>' || 'DELETE' || '</td>'
        || '<td>' || 'UNIQUECOUNTR$COUNTRYSUBDIVNAME' || '</td>'
        || '<td>' || '' || '</td>';
        
        DELETE
        FROM UNIQUECOUNTR$COUNTRYSUBDIVNAME
        ...
        MERGE /*+ USE_NL(X) */
        INTO UNIQUECOUNTR$COUNTRYSUBDIVNAME X
        USING
        (
            SELECT Name,
            Country_ID,
        ...
        (
            ...
            WHERE Table_Name IN
            (
                'UNIQUECOUNTR$COUNTRYSUBDIVNAME'
            )
            ORDER BY Table_Name
        ) LOOP
        ...
        (
            SELECT Table_Name
            FROM USER_TABLES
            WHERE Table_Name IN ('UNIQUECOUNTR$COUNTRYSUBDIVNAME')
            ORDER BY Table_Name
        ) LOOP
