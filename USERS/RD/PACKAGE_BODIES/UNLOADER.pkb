...
as
  ...
  begin
    ...
    BEGIN
      var_ddl_stmt := 'SELECT column_name FROM all_tab_columns WHERE table_name = '''||p_tab||
  ...
  begin
    ...
    begin
      ...
      -- delimited data to it.  
      --
      l_output := utl_file.fopen( p_dbdir, p_filename, 'w', 32760 );
      --
      -- Output a column header. This version uses table column comments if they
      -- exist, otherwise it defaults to the actual table column name.
      --
...
ing.');
    ...
    DBMS_OUTPUT.PUT_LINE('p_dir        = Directory we will write the .ctl and file to.');
    DBMS_OUTPUT.PUT_LINE(' OR');
    DBMS_OUTPUT.PUT_LINE('p_dbdir      = Database directory name we will write the .ctl and file to.
');
    DBMS_OUTPUT.PUT_LINE(' ');
    DBMS_OUTPUT.PUT_LINE('p_filename   = Name of file to write to. I will add .ctl to this name.
');
    ...
    DBMS_OUTPUT.PUT_LINE('p_header     = YES - Output a column header line in the file.');
