create or replace package body AWS_S3 
as
...
-- the following global settings will need to be changed for your environment
  ...
  g_aws_key                	varchar2(40); -- AWS secret key
	...
	g_wallet_pwd							varchar2(30);
  g_gmt_offset							number := 0; -- your timezone GMT adjustment
	g_aws_region							varchar2(40) := g_region_eu_london;
...
--
--------------------------------------------------------------------------------
FUNCTION ADD_LEADING_SLASH(i VARCHAR2)
RETURN VARCHAR2
AS
...
begin
	...
	if not l_valid then
		raise_application_error (-20000,
			'HTTP Method is not valid in AWS_S3.'||P_PROCEDURE);   
	end if;
...
function aws4_signing_key (
	...
	P_DATE							date,
    p_standard_region BOOLEAN DEFAULT FALSE
	) return varchar2
	------------------------------------------------------------------------------
	-- Function: 	AWS4_SIGNING_KEY
	-- Author:		Christina Moore
...
begin
	...
	l_source					:= utl_i18n.string_to_raw(CASE p_standard_region
    WHEN FALSE THEN g_aws_region
    ELSE g_region_us_standard
    END ,'AL32UTF8');
...
as
    ...
    l_canonical_url VARCHAR2(4000 BYTE) := Add_Leading_Slash(P_CANONICAL_URI);
begin
	validate_http_method(P_HTTP_METHOD,'canonical_request'); 
	l_query_string 			:= utl_url.escape(P_QUERY_STRING);
--dbms_output.put_line('l_query_string: ' || l_query_string);
	...
	if P_BUCKET is not null then
		if g_aws_region in (g_region_us_standard) then
			-- Option 1 
			l_host		:= 'host:s3.amazonaws.com';
			l_uri 		:= aws4_escape('/' || P_BUCKET || l_canonical_url);
			P_URL			:= aws4_escape('https://s3.amazonaws.com/' || P_BUCKET || l_canonical_url);
		else
			-- Option 2
			l_host	:= 'host:' || P_BUCKET || '.s3.' || g_aws_region || '.amazonaws.com';
            l_uri 		:= aws4_escape(l_canonical_url);
...
begin
	...
	l_string_to_sign := l_string_to_sign || l_date_string || '/' || CASE p_standard_region
        WHEN FALSE THEN g_aws_region
        ELSE g_region_us_standard
    END || '/s3/aws4_request' || lf;
...
begin
	l_sign_string := signature_string(
		...
		P_DATE								=> P_DATE,
        p_standard_region => p_standard_region
		);

	l_signature := aws4_signing_key
    (
        P_STRING_TO_SIGN => l_sign_string,
        P_DATE => P_DATE,
        p_standard_region => p_standard_region
    );
	return l_signature;
...
begin
	...
	l_signature := aws4_signature (
		...
		P_DATE							=> P_DATE,
        p_standard_region => CASE
            WHEN P_BUCKET IS NULL THEN TRUE
            ELSE FALSE
        END
		);
...
begin
	...
	l_signature := prep_aws_data (
		...
		P_CANONICAL_URI				=> Add_Leading_Slash(P_OBJECT),
...
/*
declare
  l_bucket_list					AWS_S3.t_bucket_list;
begin

	l_bucket_list :=	AWS_S3.get_bucket_list;
...
--dbms_output.put_line('get_bucket_list.l_canonical_request:' || l_canonical_request);
--dbms_output.put_line('get_bucket_list.l_url:' || l_url);
	apex_web_service.g_request_headers(1).name  := 'Authorization';
	apex_web_service.g_request_headers(1).value := 
		g_aws4_auth  ||
		' Credential=' || g_aws_id || '/' || l_date_string || '/' || g_region_us_standard || '/s3/aws4_request,' ||
		' SignedHeaders=host;x-amz-content-sha256;x-amz-date,' ||
		' Signature=' || l_signature ;
...
begin
	...
	l_http_method				:= 'GET';
	l_payload_hash 			:= g_null_hash;
	l_canonical_uri			:= ADD_LEADING_SLASH(P_CANONICAL_URI);
	...
	exception when others then
		...
		begin
			case
				when l_error_code IN (-29273, -31011) then
...
/*
declare
  l_object_list					AWS_S3.t_object_list;
begin

	AWS_S3.get_object_list(
...
*/
	...
	l_more							boolean := true;
  l_count             pls_integer := 0;
	l_max_keys					number := 1000; -- safety valve on loop
	l_signature					varchar2(4000);	
...
begin
	if P_BUCKET is null then
		raise_application_error (-20000,
			'Bucket Name is null in AWS_S3.get_object_url'); 
	else
...
-- manage the canonical URI
	...
	l_req_canonical := aws4_escape('/' || P_BUCKET || l_canonical_url);

		else
			-- Option 2
            l_host	:= 'host:' || P_BUCKET || '.s3.' || g_aws_region || '.amazonaws.com';
            l_url := 'https://' || P_BUCKET || '.s3.' || g_aws_region || '.amazonaws.com';			
            
	l_req_canonical := aws4_escape(l_canonical_url);
            
		end if; -- us-east-1 or not	
	
    l_url := aws4_escape(l_url || l_canonical_url);
...
begin
	...
	l_signature := prep_aws_data (
		P_BUCKET							=> P_BUCKET,
		P_HTTP_METHOD					=> l_http_method,
		P_CANONICAL_URI				=> Add_Leading_Slash(P_OBJECT_KEY),
		P_QUERY_STRING				=> l_query_string,
		P_DATE								=> l_date,
	...
	apex_web_service.g_request_headers(4).value := COALESCE(P_MIMETYPE, 'application/octet-stream');
...
BEGIN
    ...
    g_aws_id := GET_API_KEY('AWSAccessKeyId');
    
    g_aws_key := GET_API_KEY('AWSSecretKey');
...
WHEN NO_DATA_FOUND THEN
    ...
    Raise_Application_Error(-20000, 'Could not find AWS keys/Oracle Wallet');
